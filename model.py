...
def selectPositive(x,y,rate,T):
    y=y-yieldstress.solveStress(rate,T)
    index=[]
...
X=[rate,t]
popt, pcov = curve_fit(fit_k,X, k1,bounds=([-np.inf,-0.01,0],[np.inf,0.01,300]),maxfev=500000)
A1,n1,Q1=popt
popt, pcov = curve_fit(fit_k,X, k2,bounds=([-np.inf,-0.01,1000],[np.inf,0.01,np.inf]),maxfev=500000)
A2,n2,Q2=popt
...
for i in np.arange(len(strain_diff_rate)):
    ...
    stress_at_strain = np.array(stress_at_strain) + yieldstress.solveStress(diff_rate[i], 293)
...
for i in np.arange(len(strain_diff_tem)):
    ...
    stress_at_strain = np.array(stress_at_strain) + yieldstress.solveStress(0.001, diff_tem[i])
